...
export class TrayBackend {
     ...
     *
     * The `gicon` a caller may pass is deliberately ignored: the source
     * already carries the Tailscale logo in the banner header, so a second
     * copy on the notification only widens the banner. Toast mode still
     * honours the parameter, which is why it stays in the signature.
     *
     * @param {{message: string, title: string, level?: string,
     *          gicon?: Gio.Icon, onActivate?: () => void}} opts
     * @returns {TrayHandle}
     */
