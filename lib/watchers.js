...
//
// Lifted out of menu.js, where the two _maybeToast* helpers it replaced
// had no business living: they render nothing.
...
// The CLI itself appearing or vanishing on PATH — a package installed or
// removed under a running session. Everything the menu offers depends on
// it, so the whole menu empties or fills in one go, and nothing else on
// screen would say why.
//
// Cold start is deliberately silent here too: a machine that has never had
// Tailscale would otherwise report the fact at every login, and the panel
// says it already. Only a session that watched the binary go gets told.
function _installedEvents(track, snap, out) {
    if (!track.seeded)
        return;
    if (typeof track.installed !== 'boolean' || typeof snap.installed !== 'boolean')
        return;
    if (track.installed === snap.installed)
        return;
    out.push(_event(
        snap.installed ? 'tailscale-installed' : 'tailscale-missing',
        snap.installed ? 'info' : 'error',
        {},
...
export function computeEvents(track, snap) {
    ...
    const events = [];
    _installedEvents(track, snap, events);
    // A snapshot with no CLI behind it is empty by construction, not by
    // observation. Running the rules below over it would report an
    // account, an exit node and a set of capabilities all "going away"
    // when what went away is the binary — which is the event above and
    // the whole of it. The rest of the tracker carries forward untouched
    // so the return trip compares against the world as it was rather
    // than against the placeholder.
    ...
    return {
        ...
        track: {
            ...
            installed:         snap.installed ?? track.installed,
        },
    };
}
