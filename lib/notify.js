...
export const Notifier = {
     ...
     */
    beginQuiet(scope) {
        return this._policy?.beginQuiet(scope) ?? 0;
    },
    ...
    async withFeedback(category, pendingMsg, successMsg, fn) {
        ...
        try {
            ...
            // force: true — this is a user-initiated operation, not part of
            // the burst an ALL quiet window (account switch, login) exists to
            // mute. Without it, every action taken during that 6-30s window
            // is silently swallowed: no pending, no success, no error. force
            // only bypasses the quiet-window checks in shouldShow(), not the
            // category-enabled check, so a muted category still stays muted.
            let handle = this.notify({
                category, level: 'pending', message: pendingMsg, force: true,
            });
            ...
            // failure the user asked to see. force: true for the same reason as
            // the pending notify above — a quiet window must not eat a
            // user-initiated action's outcome, success or failure.
            if (handle.isNoop)
                handle = this.notify({ category, level, message, force: true });
            else
