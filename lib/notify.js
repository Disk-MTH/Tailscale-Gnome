...
export const Notifier = {
    ...
    init(settings, opts = {}) {
        ...
        const refreshCategories = () => {
            for (const [category, key] of Object.entries(CATEGORY_KEY))
                this._policy.setCategoryMode(category, settings.get_string(key));
        };
        refreshCategories();
    ...
    notify({ category, message, level = 'info',
        ...
        // destroy() drops the backend, and that missing reference is the
        // whole guard: this path must never reach this._tray.show() and
        // register a fresh MessageTray.Source with the shell post-disable.
        if (!this._tray)
            return NOOP_HANDLE;
    ...
    async withFeedback(category, pendingMsg, successMsg, fn) {
        ...
        try {
            ...
            // a category set to "errors only" drops the pending banner and
            // still wants the failure. Create the notification late in that
            // case rather than swallowing a failure the user asked to see.
            if (handle.isNoop)
