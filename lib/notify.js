...
export const Notifier = {
    ...
    get _backend() {
        return this._settings?.get_string('notification-mode') === MODE_NOTIFICATION
            ? this._tray
            : this._toast;
    },

    /**
     * Whether the live backend can act on a click. False in toast mode,
     * where the bubbles are deliberately click-through — callers use this
     * to decide whether offering "click to open" would be a lie.
     *
     * @returns {boolean}
     */
    get canActivate() {
        return this._backend === this._tray;
    },

    /**
     * `onActivate` runs when the user clicks the notification. Only the
     * notification backend can honour it: toasts are click-through chrome
     * (see toast.js), so a caller that offers one must not promise it in
     * the message text without checking which backend is live.
     *
     * @param {{category: string, message: string, level?: string,
     *          gicon?: Gio.Icon, spontaneous?: boolean,
    ...
    notify({ category, message, level = 'info', gicon = null,
        ...
        if (!this._policy.shouldShow({ category, level, spontaneous }))
            return NOOP_HANDLE;
        return this._backend.show({
            message, level, gicon, onActivate,
