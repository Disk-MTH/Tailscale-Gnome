...
const EMPTY_SNAPSHOT = Object.freeze({
    ...
    // Passive feature gates derived from `tailscale status --json`:
    //   - taildropAvailable: Self.NoFileSharingReason is empty AND
    //     Self.TaildropTarget is 0 (the daemon's "available" code).
    //   - funnelsAvailable:  Self.CapMap has the "funnel" nodeAttr key.
    // Null = unknown (status not yet fetched); UI should treat null as
    // "assume available" to avoid hiding controls before we know.
...
function _buildSnapshot(statusJson, prefsJson, accounts, canControl, funnels) {
    ...
    // Passive feature signals. The daemon publishes these in `status --json`
    // so we don't need to attempt a CLI action to discover an ACL block.
    //   NoFileSharingReason: human-readable reason; empty when allowed.
    //   TaildropTarget:      enum (0 = available, !=0 = blocked).
    //   CapMap["funnel"]:    present when the "funnel" nodeAttr was granted.
    const noFileSharingReason = self.NoFileSharingReason || '';
    const taildropTarget = self.TaildropTarget ?? 0;
    const taildropAvailable = !noFileSharingReason && taildropTarget === 0;
    const capMap = self.CapMap || {};
    const funnelsAvailable = Object.prototype.hasOwnProperty.call(capMap, 'funnel');
...
export const TailscaleClient = GObject.registerClass(
    ...
    class TailscaleClient extends GObject.Object {
        _init(params = {}) {
            ...
            // Throttle for the Taildrop availability probe (see _probeTaildrop):
            // the daemon's status JSON doesn't expose tailnet-wide "filesharing
            // disabled by admin", so we periodically reissue `file cp --targets`
            // and read the CLI response to keep the GSetting fresh.
            this._lastTaildropProbe = 0;
            this._taildropProbeInflight = false;
        }
        ...
        // Probe Taildrop availability by running `file cp --targets` in the
        // background. Throttled to at most once per minute. The CLI errors
        // out with a clear "filesharing/taildrop disabled" message when an
        // admin has turned the feature off tailnet-wide, which is the one
        // case our passive snapshot signals can't catch (see comment in
        // refresh()). The result lands in the feature-taildrop-available
        // GSetting via fileTargets() which the prefs UI already watches.
        _probeTaildrop() {
            if (this._destroyed) return;
            if (this._taildropProbeInflight) return;
            const now = GLib.get_monotonic_time() / 1_000_000;
            if (now - this._lastTaildropProbe < 60) return;
            this._lastTaildropProbe = now;
            this._taildropProbeInflight = true;
            // Discard the returned target list — only the GSetting side
            // effect matters here.
            this.fileTargets()
                .catch(() => {})
                .finally(() => { this._taildropProbeInflight = false; });
        }
        ...
        async refresh() {
            ...
            try {
                ...
                if (!status.ok) {
                    ...
                    // Funnel cap is authoritative from status JSON.
                    if (snap.funnelsAvailable !== null)
                        this._setAvailability('feature-funnels-available', snap.funnelsAvailable);
                    // Taildrop: only trust the passive signal when it says
                    // "blocked" — Self.NoFileSharingReason stays empty when
                    // an admin disables filesharing tailnet-wide, so we still
                    // need the periodic CLI probe to catch that case.
                    if (snap.taildropAvailable === false)
        ...
        async fileTargets() {
            ...
            // Success path: the daemon answered, so Taildrop is allowed.
...
function _snapshotEqual(a, b) {
    ...
    if ((a.noFileSharingReason || '') !== (b.noFileSharingReason || '')) return false;
