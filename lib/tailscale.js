...
function _buildSnapshot(statusJson, prefsJson, accounts) {
    if (!statusJson) {
        return { ...EMPTY_SNAPSHOT, accounts, error: 'no-status' };
    }
...
export const TailscaleClient = GObject.registerClass(
    ...
    class TailscaleClient extends GObject.Object {
        ...
        async _fetchAccounts() {
            ...
            if (!r.ok) return [];
            return _parseSwitchList(r.stdout);
        }
        ...
        async refresh() {
            ...
            this._inflight = true;
            try {
                const [status, prefs, accounts] = await Promise.all([
                    this._fetchStatus(),
                    this._fetchPrefs(),
                    this._fetchAccounts(),
                ]);
                let snap;
                if (!status.ok) {
                    snap = { ...EMPTY_SNAPSHOT, accounts, error: status.err };
                } else {
                    snap = _buildSnapshot(status.json, prefs, accounts);
                }
        ...
        /**
         * Run a state-changing command and refresh afterwards.
         * @returns {Promise<{ok: boolean, message: string}>}
         */
        async _runAndRefresh(args, { quiet = false } = {}) {
            const r = await this._run(args);
            if (!r.ok && !quiet) {
                this.emit('error', `tailscale ${args.join(' ')}: ${r.stderr.trim() || `exit ${r.code}`}`);
            }
            // Always refresh — even on failure, the daemon may have moved.
            await this.refresh();
            return { ok: r.ok, message: r.ok ? r.stdout.trim() : r.stderr.trim() };
        }

        up()           { return this._runAndRefresh(['up', '--reset']); }
        ...
        switchAccount(id) {
            if (!id) return Promise.resolve({ ok: false, message: 'missing account id' });
            return this._runAndRefresh(['switch', id]);
        }
