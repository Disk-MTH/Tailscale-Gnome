...
// launch site in the extension, and it also holds BIN and the PATH probe
// so the preferences process gets the same answers this one does.
import {
...
export const TailscaleClient = GObject.registerClass(
    ...
    class TailscaleClient extends GObject.Object {
        ...
        // right here: PKEXEC and BIN are both fixed strings in spawn.js,
        // not settings.
        async _runPriv(args) {
            try {
        ...
        async _fetchFunnels() {
            ...
            if (!r.ok) return [];
            try {
                return parseFunnels(JSON.parse(r.stdout));
            } catch {
                return [];
            }
        }
