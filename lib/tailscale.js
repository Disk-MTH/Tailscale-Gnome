...
export const TailscaleClient = GObject.registerClass(
    ...
    class TailscaleClient extends GObject.Object {
        ...
        /**
         * Login is interactive: `tailscale login` prints an auth URL on stdout
         * then blocks until the browser flow completes. We read lines
         * asynchronously, find the https:// URL, open it in the default
         * browser, and let the process keep running in the background.
         */
        async login() {
            let proc;
            try {
                ...
                dis.read_line_async(
                    ...
                    (obj, res) => {
                        try {
                            ...
                            if (line === null) {
                                // EOF: auth completed (or process exited)
                                this.refresh().catch(() => {});
                                return;
                            }
                            if (!urlFound) {
                                ...
                                if (m) {
                                    ...
                                    // Emit as info via the error channel so the
                                    // notification surface is aware; prefix makes
                                    // it clear it is not an error.
                                    this.emit('error',
                                        `Login: browser opened. If not, visit: ${url}`);
                                }
                            }
                            readNext();
                        } catch (e) {
        ...
        async switchAccount(id) {
            ...
            // Use _run (not _runAndRefresh) for every intermediate step so we
            // emit exactly ONE state-changed at the very end. Multiple
            // _runAndRefresh calls in sequence cause visible blinks in the menu
            // because each intermediate refresh emits a transient state.
            ...
            // Peek at the new profile's state without emitting yet.
            const [stR, prR] = await Promise.all([
                this._run(['status', '--json']),
                this._run(['debug', 'prefs']),
            ]);
            let newRunning = false, needsLogin = false, loggedOut = false;
            try {
                const j = JSON.parse(stR.stdout);
                newRunning  = j.BackendState === 'Running';
                needsLogin  = j.BackendState === 'NeedsLogin';
            } catch {}
            try { loggedOut = !!JSON.parse(prR.stdout)?.LoggedOut; } catch {}

            // Restore the previous running state silently.
            if (wasRunning && !newRunning) {
                if (loggedOut || needsLogin) {
                    await this.login();
                } else {
                    await this._run(['up']);
                }
            } else if (!wasRunning && newRunning) {
                await this._run(['down']);
            }

            // Single final refresh → single UI update, no blink.
            await this.refresh();
