...
export const TailscaleClient = GObject.registerClass(
    ...
    class TailscaleClient extends GObject.Object {
        ...
        /**
         * One-shot Taildrop + Funnel availability probe. Same logic as
         * the prefs "Check availability" buttons; called once at startup
         * and on account changes so the cached gsettings flags stay in
         * sync with the current tailnet's ACL. Best-effort: any failure
         * leaves the cached flag untouched so a transient daemon hiccup
         * doesn't grey out the UI.
         */
        async probeAvailability() {
            if (this._destroyed || !this._settings) return;

            // Taildrop: `file cp --targets` succeeds (allowed) or prints
            // a specific "filesharing disabled" diagnostic (denied).
            try {
                const r = await this._run(['file', 'cp', '--targets']);
                if (r.ok) {
                    this._settings.set_boolean('feature-taildrop-available', true);
                } else {
                    ...
                    if (blocked)
                        this._settings.set_boolean('feature-taildrop-available', false);
                }
            } catch (_) {}

            // Funnel: presence of the "funnel" nodeAttr in Self.CapMap is
            // the authoritative signal — the daemon already published it.
            try {
                const r = await this._run(['status', '--json']);
                if (r.ok) {
                    const j = JSON.parse(r.stdout);
                    const capMap = j?.Self?.CapMap || {};
                    const ok = Object.prototype.hasOwnProperty.call(capMap, 'funnel');
                    this._settings.set_boolean('feature-funnels-available', ok);
                }
            } catch (_) {}
        }
