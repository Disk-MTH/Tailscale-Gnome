...
function _buildSnapshot(statusJson, prefsJson, accounts, canControl) {
    if (!statusJson) {
        return { ...EMPTY_SNAPSHOT, accounts, canControl, error: 'no-status' };
    }
...
export const TailscaleClient = GObject.registerClass(
    ...
    class TailscaleClient extends GObject.Object {
        ...
        async refresh() {
            ...
            this._inflight = true;
            try {
                const [status, prefs, accountsResult] = await Promise.all([
                    this._fetchStatus(),
                    this._fetchPrefs(),
                    this._fetchAccounts(),
                ]);
                ...
                let snap;
                if (!status.ok) {
                    snap = { ...EMPTY_SNAPSHOT, accounts, canControl, error: status.err };
                } else {
                    snap = _buildSnapshot(status.json, prefs, accounts, canControl);
                }
        ...
        async switchAccount(id) {
            ...
            const nowRunning = this._snapshot.running;
            if (wasRunning && !nowRunning)      await this._runAndRefresh(['up']);
            else if (!wasRunning && nowRunning) await this._runAndRefresh(['down']);
            return result;
        }
