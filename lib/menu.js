...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        ...
        async _runAddFunnelFlow() {
            ...
            // Short-circuit when the daemon already told us Funnel is not
            // granted by ACL (no "funnel" nodeAttr in Self.CapMap). Avoids
            // a spurious "is not enabled" round-trip and points the user at
            // the admin console directly.
            const snap = this._client.snapshot;
            if (snap?.funnelsAvailable === false) {
                ToastManager.show({
                    level: "error",
                    message: _("Funnel is not enabled for this tailnet. Ask an admin to grant the \"funnel\" attribute."),
                });
                return;
            }
        ...
        async _runSendFlow(preselectedFiles) {
            ...
            // Prefer the passive signal: the daemon already knows whether
            // Taildrop is allowed for this node (Self.NoFileSharingReason),
            // so we can short-circuit before invoking the CLI.
            const snap = this._client.snapshot;
            if (snap?.taildropAvailable === false) {
                ToastManager.show({
                    level: "error",
                    message: snap.noFileSharingReason ||
                        _("Taildrop is disabled for this tailnet by your admin."),
                });
                return;
            }
