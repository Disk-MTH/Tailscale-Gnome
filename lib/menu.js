...
const ICON_COPY = 'action-copy-symbolic';
const ICON_ACCOUNT = 'account-symbolic';
const ICON_TAILNET = 'tailnet-symbolic';
const ICON_TRASH = 'action-trash-symbolic';
const ICON_REFRESH = 'action-refresh-symbolic';
const ICON_EXIT_WARN = 'vpn-disconnected-symbolic';
 ...
 *
 * @param {{icon: Gio.Icon, targets: {label: string, value: string}[],
 *          onCopy: (value: string) => void}} opts
 * @returns {{button: St.Button, chooser: St.BoxLayout}}
 */
function _makeCopyControl({ icon, targets, onCopy }) {
    ...
    const button = new St.Button({
        ...
        child: new St.Icon({ gicon: icon, icon_size: 16 }),
        y_align: Clutter.ActorAlign.CENTER,
        can_focus: true,
...
const SelfRow = GObject.registerClass(
    class SelfRow extends PopupMenu.PopupBaseMenuItem {
         ...
         *          onCopy: (value: string) => void}} opts
         */
        update({ title, subtitle, online, copyIcon, copyTargets, onCopy }) {
            this._title.text = title;
...
const SendFileDialog = GObject.registerClass(
    class SendFileDialog extends ModalDialog.ModalDialog {
        ...
        _makeRow(peer) {
            ...
            row.add_child(text);
            // The dialog only ever lists reachable peers.
            row.add_child(_makeStatusDot(true));
            btn.set_child(row);
...
// In-shell funnel creation dialog, same visual family as SendFileDialog.
// One numeric entry for the local port to expose, plus a radio-pill row
// for the public Funnel port (Tailscale only allows 443/8443/10000 —
// snapshot.funnelPorts). Ports that already carry a funnel are greyed
// out: replacing one silently would overwrite its serve config, so the
// user has to remove it first.
const PortDialog = GObject.registerClass(
    class PortDialog extends ModalDialog.ModalDialog {
        _init({ ports, usedPorts, onPick }) {
            super._init({ styleClass: 'tailscale-send-dialog' });
            this._onPick = onPick;
            this._resolved = false;  // pick-once latch
            ...
            this.contentLayout.add_child(new St.Label({
                style_class: 'tailscale-send-subtitle',
                text: _('Local port to expose'),
            }));

            // Local on purpose: the entry lives and dies with the dialog's
            // actor tree, no teardown bookkeeping needed.
            ...
            const commit = () => this._finish({
                localText: entry.get_text(),
                httpsPort: this._selectedPort,
            });
            entry.clutter_text.connectObject('activate', commit, this);
            ...
            const portRow = new St.BoxLayout({
                style_class: 'tailscale-port-choices',
                x_expand: true,
            });
            for (const port of ports) {
                const used = usedPorts.has(port);
                const btn = new St.Button({
                ...
                portRow.add_child(btn);
                this._portButtons.set(port, btn);
            }
            this.contentLayout.add_child(portRow);
            const firstFree = ports.find((p) => !usedPorts.has(p));
            if (firstFree !== undefined) this._selectPort(firstFree);
            ...
            this.setInitialKeyFocus(entry.clutter_text);
        }
...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        _init({ extension, client }) {
            ...
            // Prime the stock header first: its labels must exist before
            // _attachHeaderIdentity can find them by shape.
            this.menu.setHeader(
            ...
            this._attachHeaderIdentity();
            this._setHeaderIdentity(
                _gicon(extension, ICON_DISABLED),
                'Tailscale',
                _('Disconnected'),
            );
        ...
        _attachHeaderRefreshButton() {
            ...
            this._headerRefreshBtn = new St.Button({
                ...
                child: new St.Icon({
                    gicon: _gicon(this._extension, ICON_REFRESH),
                    icon_size: 16,
                }),
                y_align: Clutter.ActorAlign.CENTER,
        ...
        // Replace the header's stock title/subtitle pair with two labelled
        // lines: the signed-in account, then the tailnet it belongs to.
        //
        // The label box is found by shape rather than by name — it is the
        // child of the header that stacks the two St.Labels — because the
        // property holding it is private to the Shell and has no stability
        // guarantee. When the shape does not match, _setHeaderIdentity falls
        // back to the public setHeader(), which still shows the same two
        // strings, just without the per-line icons.
        _attachHeaderIdentity() {
            const header = this.menu._header;
            if (!header || typeof header.get_children !== 'function') return;

            const labelBox = header.get_children().find(
                (child) =>
                    child !== this._headerRefreshBtn &&
                    typeof child.get_children === 'function' &&
                    child.get_children().filter((c) => c instanceof St.Label)
                        .length >= 2,
            );
            if (!labelBox) return;

            // Remember what we hid so destroy() can put it back: the header
            // outlives this toggle.
            this._hiddenHeaderLabels = labelBox.get_children()
                .filter((c) => c.visible);
            for (const label of this._hiddenHeaderLabels)
                label.visible = false;

            const makeLine = (iconName, styleClass) => {
                const line = new St.BoxLayout({
                    style_class: 'tailscale-header-line',
                    y_align: Clutter.ActorAlign.CENTER,
                });
                line.add_child(new St.Icon({
                    gicon: _gicon(this._extension, iconName),
                    icon_size: 14,
                    style_class: 'tailscale-header-line-icon',
                    y_align: Clutter.ActorAlign.CENTER,
                }));
            ...
            this._headerAccount = makeLine(ICON_ACCOUNT, 'tailscale-header-account');
            this._headerTailnet = makeLine(ICON_TAILNET, 'tailscale-header-tailnet');
        }

        // Drive the header. With the custom lines in place the icon still
        // comes from setHeader(); without them setHeader() carries the text
        // too, so the information is never lost.
        _setHeaderIdentity(icon, account, tailnet) {
            if (!this._headerAccount) {
                this.menu.setHeader(icon, account, tailnet);
                return;
            }
            this.menu.setHeader(icon, '', '');
            // setHeader() drives the visibility of the labels it owns, so
            // re-assert ours after every call rather than only at attach.
            for (const label of this._hiddenHeaderLabels ?? [])
                label.visible = false;
            this._headerAccount.label.text = account;
            this._headerAccount.line.visible = !!account;
            this._headerTailnet.label.text = tailnet;
            this._headerTailnet.line.visible = !!tailnet;
        }
        ...
        _buildMenu() {
            ...
            this._exitNodeWarnIcon = new St.Icon({
                gicon: _gicon(this._extension, ICON_EXIT_WARN),
            ...
            this._sendFileRow = new ActionRow(_('Send file'),
                () => { this._closeAllMenus(); this.runSendFlow(); });
        ...
        _render(snap) {
            ...
            // Identity over hardware: which account is signed in and which
            // tailnet it reaches is what the user acts on here. The device's
            // own name moved to the self row, where its IP already lives.
            ...
            this._setHeaderIdentity(
                ...
                current?.account || snap.accountName || _('No account'),
                current?.tailnet || snap.magicDNSSuffix || subtitle,
            );
            ...
            this._selfRow.update({
                ...
                online: snap.running,
                copyIcon: _gicon(this._extension, ICON_COPY),
        ...
        _makeFunnelRow(f, url) {
            ...
            const copyBtn = new St.Button({
                style_class: 'button tailscale-icon-btn',
                child: new St.Icon({
                    gicon: _gicon(this._extension, ICON_COPY),
                    icon_size: 16,
                }),
                y_align: Clutter.ActorAlign.CENTER,
            });
            ...
            const removeBtn = new St.Button({
                style_class: 'button tailscale-icon-btn',
                child: new St.Icon({
                    gicon: _gicon(this._extension, ICON_TRASH),
                    icon_size: 16,
                }),
                y_align: Clutter.ActorAlign.CENTER,
            });
            // Toasts name the public port: now that it is user-picked it
            // is the funnel's identity (it's the port in the URL), and it
            // matches the "Add funnel" wording.
            removeBtn.connect('clicked', () => this._withFeedback(
                Category.FUNNEL,
                _fmt(_('Removing funnel on port %d'), f.httpsPort),
                _fmt(_('Funnel removed on port %d'), f.httpsPort),
        ...
        async _runAddFunnelFlow() {
            const snap = this._client.snapshot;
            const usedPorts = new Set(snap.funnels.map((f) => f.httpsPort));
            if (snap.funnelPorts.every((p) => usedPorts.has(p))) {
                Notifier.notify({
                    category: Category.FUNNEL,
                    level: 'error',
                    message: _fmt(
                        _('All Funnel ports are in use (%s) — remove one first'),
                        snap.funnelPorts.join(', ')),
                });
                return;
            }
            const pick = await new Promise((resolve) => {
                this._showDialog(new PortDialog({
                    ports: snap.funnelPorts,
                    usedPorts,
                    onPick: resolve,
                }));
            });
            if (!pick || !pick.localText.trim()) return;
            const localPort = parseInt(pick.localText.trim(), 10);
            ...
            const httpsPort = pick.httpsPort;
            ...
            const r = await this._withFeedback(
                Category.FUNNEL,
                _fmt(_('Adding funnel on port %d'), httpsPort),
                _fmt(_('Funnel added on port %d'), httpsPort),
                async () => {
        ...
        // Send flow: XDG portal file picker (skipped if files pre-selected,
        // e.g. from the Nautilus DBus call) → in-shell native peer picker →
        // Taildrop send. Peers come from `tailscale file cp --targets`,
        // which only lists nodes that can actually receive Taildrop files.
        async runSendFlow(preselectedFiles) {
            let files = preselectedFiles;
            if (!files || files.length === 0) {
                files = await this._pickFiles(_('Send files via Taildrop'));
                if (!files) return;
            }
        ...
        // Feedback toasts live in Notifier (shared with extension.js); the
        // alias just keeps the many call sites above short.
        ...
        destroy() {
            ...
            // The header belongs to the Shell, not to this toggle, so the
            // actors we grafted onto it are not torn down with us. Remove
            // them by hand and restore the stock labels we hid, or the next
            // enable() would stack a second identity block on top.
            for (const part of [this._headerAccount, this._headerTailnet]) {
                part?.line.destroy();
            }
            this._headerAccount = null;
            this._headerTailnet = null;
            this._headerRefreshBtn?.destroy();
            this._headerRefreshBtn = null;
            for (const label of this._hiddenHeaderLabels ?? [])
                label.visible = true;
            this._hiddenHeaderLabels = null;
