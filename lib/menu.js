...
// A bounded scrolling wrapper used to keep the QuickToggleMenu from
// growing past the screen. We add a normal PopupBaseMenuItem (which IS
// an actor and integrates cleanly with the parent menu) and put an
// St.ScrollView inside it. A PopupMenuSection lives inside the scroll
// view so callers keep using the familiar addMenuItem() API.
const ScrollableMenuItem = GObject.registerClass(
    class ScrollableMenuItem extends PopupMenu.PopupBaseMenuItem {
        _init(maxHeightPx) {
            super._init({
                reactive: false,
                can_focus: false,
                style_class: "tailscale-scroll-wrap",
            ...
            this._scrollView.set_child(this.section.actor);
...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        _init({ extension, client }) {
            ...
            // Feature toggles (Features panel in prefs). Re-render so
            // newly enabled blocks appear and disabled ones disappear.
            for (const key of [
                "feature-exit-nodes",
                "feature-dns",
                "feature-routes",
        ...
        _attachHeaderRefreshButton() {
            ...
            this._headerRefreshBtn = new St.Button({
                style_class: "icon-button flat tailscale-header-btn",
                child: new St.Icon({
        ...
        _buildMenu() {
            ...
            // Scrollable middle. Everything between the operator row and the
            // bottom Settings/Admin row lives inside this item so the popup
            // can't grow past the monitor edge. addMenuItem on the inner
            // section preserves the usual menu semantics (signals, focus).
            const monitor = Main.layoutManager.primaryMonitor;
            const panelH = Main.panel?.height ?? 32;
            const maxH = monitor
                ? Math.floor((monitor.height - panelH * 2) * 0.70)
                : 520;
            this._scrollWrap = new ScrollableMenuItem(maxH);
            this.menu.addMenuItem(this._scrollWrap);
            const sec = this._scrollWrap.section;

            this._ipRow = new InfoRow(_("IP"), "-");
            sec.addMenuItem(this._ipRow);

            this._accountsSubMenu = new PopupMenu.PopupSubMenuMenuItem(
                _("Account"),
                true,
            );
            sec.addMenuItem(this._accountsSubMenu);

            this._sep1 = new PopupMenu.PopupSeparatorMenuItem();
            sec.addMenuItem(this._sep1);
            ...
            sec.addMenuItem(this._peersSubMenu);
            ...
            sec.addMenuItem(this._funnelSubMenu);

            this._sep3 = new PopupMenu.PopupSeparatorMenuItem();
            sec.addMenuItem(this._sep3);
        ...
        _onUserClick() {
            ...
            if (!ready) {
                ...
                if (!snap.canControl) {
                    ...
                    Main.notify(
                        "Tailscale",
                        _("Login required (see Account menu)"),
                    );
                    this.menu.open();
                }
                return;
            }
            if (this.checked) this._client.up();
            else this._client.down();
        }
        ...
        _render(snap) {
            ...
            if (!snap.canControl) {
                ...
                this._banner.visible = false;
                this._scrollWrap.visible = false;
        ...
        _renderFunnels(snap) {
            ...
            sub.addMenuItem(new ActionRow(_("Add funnel"),
                () => { this.menu.close(); this._runAddFunnelFlow(); }));
        }
        ...
        _makeFunnelRow(f, url) {
            ...
            removeBtn.connect("clicked", () => this._client.removeFunnel(f.httpsPort));
        ...
        async _runAddFunnelFlow() {
            ...
            if (isNaN(port) || port < 1 || port > 65535) {
                Main.notifyError("Tailscale", _("Invalid port number"));
                return;
            }
            const r = await this._client.addFunnel(port);
            if (r.notEnabled) {
                try { Gio.AppInfo.launch_default_for_uri(r.url, null); } catch (_) {}
                Main.notify("Tailscale",
                    _("Approve Funnel in the browser, then click Add funnel again."));
            } else if (!r.ok) {
                Main.notifyError("Tailscale", r.message);
            } else {
                Main.notify("Tailscale", _("Funnel added"));
            }
        }
        ...
        // Honor the per-feature prefs toggles. Called after the main render
        // pass has marked all items visible; this trims down to what the
        // user has enabled. The Send/Funnel separator hides when both
        ...
        // Send flow: zenity file picker → zenity peer picker → cp.
        // The peer picker shows hostnames but sends to IPs -hostnames can
        // contain spaces / collisions across tailnets, IPs are unambiguous.
        async _runSendFlow(preselectedFiles) {
            ...
            const targets = await this._client.fileTargets();
            const online = targets.filter((t) => !t.offline);
            if (online.length === 0) {
                Main.notifyError("Tailscale",
                    _("No online peers available to receive files"));
                return;
            }

            const peerArgs = [
                "zenity", "--list",
                "--title=Send to which peer?",
                "--column=Peer", "--column=IP",
                "--print-column=2",
            ];
            for (const t of online) {
                peerArgs.push(t.host);
                peerArgs.push(t.ip);
            }
            const peerRes = await _spawnAsync(peerArgs);
            if (!peerRes.ok || !peerRes.stdout.trim()) return;
            const ip = peerRes.stdout.trim().split("|")[0];

            await this._client.sendFile(ip, files);
        }
