...
import {
    BannerRow, ICON_ACTIVE, ICON_COPY, ICON_DISABLED, InfoRow, PeerRow,
...
export function statusText(snap) {
    // Ahead of the generic error: "unavailable" reads as a daemon that is
    // down, which is a thing you wait out. Nothing here resolves itself.
    if (snap.installed === false) return _('Tailscale is not installed');
...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        ...
        /**
         * Gate for the entry points that bypass the menu — keybindings and
         * the Nautilus D-Bus call. Hiding a row stops nothing that does not
         * go through the row.
         *
         * @param {string} category one of Category, for the refusal
         * @returns {boolean} true when the caller may proceed
         */
        _requireInstalled(category) {
            if (this._client.snapshot.installed !== false) return true;
            Notifier.notify({
                category,
                level: 'error',
        ...
        _onUserClick() {
            ...
            // Nothing to switch on. Put the switch back where it was and
            // open the menu, where the banner says why in a full sentence
            // instead of a banner that vanishes in four seconds.
        ...
        _render(snap) {
            ...
            // No CLI on PATH: every row below drives a command that cannot
            // run, so the menu is stripped to the one line that explains
            // it. This gate is first because it outranks the others —
            // there is no operator to set and no account to log into
            // without the binary that would do either.
            if (snap.installed === false) {
                ...
                this._banner.set(
                    ...
                    _('Install the tailscale package, then this menu comes back on its own.'),
                );
                this._banner.visible = true;
                this._operatorRow.visible = false;
                for (const item of this._mainItems) item.visible = false;
                // The admin console is a web page and would still open, but
                // it administers a tailnet this machine is not on. It goes
                // with the rest; Extension settings stays, as it does under
                // the operator gate.
        ...
        _runFunnelsFlow() {
            const snap = this._client.snapshot;
            if (!this._requireInstalled(Category.FUNNEL)) return;
        ...
        async runSendFlow(preselectedFiles) {
            ...
            // the wrong problem.
            if (!this._requireInstalled(Category.TAILDROP)) return;
