...
import { backendStatus } from './tailscale-parse.js';
import {
    fmt as _fmt, gicon as _gicon, showInFileManager as _showInFileManager,
} from './util.js';
import {
    ICON_ACTIVE, ICON_COPY, ICON_DISABLED, RoutesSubToggle, SelfRow,
    ToggleRow, copyTargetsFor, decorateWithPill,
} from './menu/rows.js';
import {
...
import {
    renderAccounts, renderExitNodes, renderPeers, renderRoutes,
...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        _init({ extension, client }) {
            ...
            // Two things ride on the menu opening and closing.
            //
            // Opening asks for a full refresh: the poll behind the panel
            // icon skips the profile list and the serve config, since
            // neither is visible with the menu shut. This is the moment
            // they become visible, and the rows render from the last
            // snapshot meanwhile, so the answer lands under an already
            // populated menu rather than an empty one.
            //
            // Closing folds an expanded chooser away with the menu that
            // holds it. Nothing else resets it: the rows outlive a
            // close/open cycle when no state change happens in between, so
            // the chooser would still be hanging open the next time the
            // panel is opened.
            this._menuStateId = this.menu.connect(
                'open-state-changed', (_m, isOpen) => {
                    ...
                    if (this._openCopyKey === null) return;
                    this._openCopyKey = null;
        ...
        _render(snap) {
            ...
            if (needsLogin) {
                ...
                renderAccounts({
                    submenu:         this._accountsSubMenu,
                    client:          this._client,
                    closeMenus:      () => this._closeAllMenus(),
                    makeOperatorRow: () => this._makeOperatorRow(),
                }, snap);
                return;
            }
            ...
            renderPeers({
                submenu:      this._peersSubMenu,
                pill:         this._peersPill,
                openCopyKey:  this._openCopyKey,
                onCopy:       (value) => this._copyToClipboard(value),
                onCopyToggle: (key) => { this._openCopyKey = key; },
            }, snap);
            renderExitNodes({
                submenu: this._exitNodeSubMenu,
                pill:    this._exitNodePill,
                client:  this._client,
            }, snap);
            renderRoutes(this._routesToggle, snap);
