...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        ...
        // Wrap menu.box in a ScrollView so the menu scrolls instead of
        // bleeding off-screen. The actor tree isn't connected at _init()
        // time, so we hook both 'parent-set' and 'open-state-changed' as
        // wrap triggers and the first one that lands wins.
        _makeMenuScrollable() {
            ...
            const wrap = () => {
                ...
                if (!parent || parent instanceof St.ScrollView) return false;
                wrapped = true;
            ...
            this._parentSetId = box.connect("parent-set", () => {
                if (wrap() && this._parentSetId) {
                    box.disconnect(this._parentSetId);
                    this._parentSetId = 0;
                }
            });
            this._scrollSignalId = this.menu.connect(
                "open-state-changed",
                (_m, isOpen) => {
                    if (isOpen) wrap();
                },
        ...
        destroy() {
            ...
            if (this._scrollSignalId)
                this.menu.disconnect(this._scrollSignalId);
            if (this._parentSetId) this.menu.box.disconnect(this._parentSetId);
            this._signalIds = [];
            this._settingsIds = [];
