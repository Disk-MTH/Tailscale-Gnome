...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        ...
        _buildMenu() {
            ...
            this._operatorRow = new PopupMenu.PopupBaseMenuItem({ reactive: false, can_focus: false });
            const opBox = new St.BoxLayout({ x_expand: true });
            opBox.add_child(new St.Label({
                text: _('Operator not set'),
                x_expand: true,
                y_align: Clutter.ActorAlign.CENTER,
            }));
            const opBtn = new St.Button({ label: _('Set operator'), style_class: 'button' });
        ...
        _onUserClick() {
            ...
            if (!ready) {
                ...
                if (!snap.canControl) {
                    ...
                    Main.notify('Tailscale', _('Login required (see Account menu)'));
                    this.menu.open();
                }
                return;
            }
        ...
        _renderExitNodes(snap) {
            ...
            let pill;
            if (isAuto) {
                // Only show the node name when it is actually online and routing.
                // If auto has no active selection (or the selected node went
                // offline), fall back to just "Auto" so the pill doesn't lie.
                const node = snap.currentExitNode;
                const name = node?.online
                    ? (node.hostname || node.dnsName?.split('.')[0] || '')
                    : '';
                pill = name ? `Auto (${name})` : _('Auto');
            } else if (snap.currentExitNode) {
                // Direct mode: keep showing the hostname even when offline —
                // the daemon still remembers the preference and will route
                // through it again as soon as the node comes back.
        ...
        _renderAccounts(snap) {
            ...
            if (snap.accounts.length === 0) {
                if (snap.accountName) {
                    const row = new PeerRow({ title: snap.accountName, checked: true });
                    row.reactive = false;
                    sub.addMenuItem(row);
                }
                if (!snap.canControl) {
