...
// Decorate a PopupSubMenuMenuItem with an icon button past the dropdown
// arrow. PopupSubMenuMenuItem adds its arrow last, so appending lands
// beyond it — on the far side of everything that opens the submenu, which
// is what makes the button read as a second, independent action rather
// than as part of the disclosure. The button swallows its own release, so
// pressing it does not also toggle the submenu underneath.
function _decorateWithAction(submenuItem, iconName, name, onClick) {
...
const SendFileDialog = GObject.registerClass(
    class SendFileDialog extends ModalDialog.ModalDialog {
        _init({ extension, paths = [], peers, pickFiles, onPick }) {
            ...
            // in the order they were ticked, and the toasts follow.
...
const PortDialog = GObject.registerClass(
    class PortDialog extends ModalDialog.ModalDialog {
        _init({ extension, ports, usedPorts, onPick }) {
            super._init({ styleClass: 'tailscale-send-dialog' });
            this._onPick = onPick;
            this._resolved = false;  // pick-once latch

            this.contentLayout.add_child(
                _dialogTitle(extension, _('Add funnel')));
...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        ...
        _buildMenu() {
            ...
            // Adding a funnel used to mean opening the submenu and finding
            // the last row in it. It is the only thing anyone does here that
            // is not about an existing funnel, so it sits on the row itself
            // and costs one click instead of two.
            _decorateWithAction(
                this._funnelSubMenu, 'list-add-symbolic', _('Add funnel'),
        ...
        async _runAddFunnelFlow() {
            const snap = this._client.snapshot;
            // Reachable with the menu entry hidden: the keyboard shortcut
            // fires wherever the user is. Saying so beats a port dialog that
            // leads to a refusal from the control plane — the same answer
            // runSendFlow gives when Taildrop is off for the tailnet.
