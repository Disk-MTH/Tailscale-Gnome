...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        _init({ extension, client }) {
            ...
            // (prefs dialog, dconf-editor), when a feature toggle flips in
            // the prefs Features panel, or when the per-tailnet availability
            // cache moves — so newly enabled blocks appear and
            // admin-disabled ones disappear without waiting for the next
            // poll.
            const renderKeys = [
                'taildrop-accept',
                'feature-exit-nodes',
            ...
            // While a user-initiated op is in flight, ToastManager owns the
            ...
            //
            // _exitTrack keeps the exit node state from the last render so
            // spontaneous changes (node offline, admin disable, auto-mode
            // loss) trigger a toast without waiting for the user to notice
            // the pill changed. _lastBackendState / _connToast do the same
            // for the daemon's connection phase (see _maybeToastConnection).
            this._exitTrack = null;
            this._lastBackendState = null;
            this._connToast = null;
            const spontaneous = (level, msg) => {
                if (ToastManager.hasActiveOp) return;
                ToastManager.show({ level, message: msg });
            };

            this._client.connectObject(
                'state-changed', (_c, snap) => this._render(snap),
                'error', (_c, msg) => spontaneous('error', msg),
                'notify-info', (_c, msg) => spontaneous('success', msg),
                this,
            );
        ...
        // Spontaneous connection-progress feedback. User-initiated up/down
        // already run behind withFeedback; this covers the transitions the
        // user didn't click for — typically tailscaled still establishing
        // the session right after login/boot — so "it's working on it, not
        // done yet" is visible without opening the menu. The pending toast
        // is sticky and resolves in place once the daemon leaves Starting.
        _maybeToastConnection(snap) {
            const prev = this._lastBackendState;
            this._lastBackendState = snap.backendState;
            if (prev === snap.backendState) return;

            if (snap.backendState === 'Starting') {
                // Only the creation is suppressed during a user-initiated
                // op (withFeedback owns that feedback); an already-shown
                // toast below always resolves, sticky pendings must never
                // outlive their Starting phase.
                if (ToastManager.hasActiveOp) return;
                const message = _('Connecting Tailscale — this may take a moment');
                if (this._connToast) this._connToast.update({ level: 'pending', message });
                else this._connToast = ToastManager.show({ level: 'pending', message });
                return;
            }
            if (!this._connToast) return;
            if (snap.backendState === 'Running') {
                this._connToast.update({
                    level: 'success',
                    message: _('Tailscale connected'),
                });
            } else {
                // Starting ended without reaching Running (stopped, login
                // needed, …): reuse the status wording the pill shows.
                this._connToast.update({
                    level: 'info',
                    message: this._statusText(snap),
                });
        ...
        _render(snap) {
            ...
            this._maybeToastConnection(snap);
            ...
            this._ipCopyBtn.visible = !!this._selfIp;
        ...
        async _runAddFunnelFlow() {
            ...
            if (openedApproval) {
                try {
                    ...
                    ToastManager.show({
                        level: 'error',
                        message: `Could not open ${openedApproval}`,
                    });
                }
                ToastManager.show({
                    level: 'info',
        ...
        _renderAccounts(snap) {
            ...
            if (snap.accounts.length === 0) {
                ...
                if (!snap.canControl) {
                    // Without operator the daemon refuses `switch --list`
                    // and `switch`, so a multi-account user would be stuck
                    // right after a logout (the pref went away with the
                    // discarded profile). One click re-grants it — a
                    // single, user-initiated pkexec prompt — and the
                    // account list appears on the follow-up refresh.
        ...
        // Feedback toasts live in ToastManager (shared with extension.js);
        // the alias just keeps the many call sites above short.
        _withFeedback(pending, success, fn) {
            return ToastManager.withFeedback(pending, success, fn);
        }
