...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        ...
        _buildMenu() {
            ...
            // meaning when an exit node is active). Created here so _render
            // can update it without re-creating it on every poll.
            this._allowLanRow = new ToggleRow(_('Allow LAN access'),
        ...
        _render(snap) {
            ...
            for (const r of [this._acceptDNSRow, this._acceptRoutesRow,
                this._shieldsUpRow, this._runSSHRow, this._allowLanRow])
        ...
        _renderFunnels(snap) {
            ...
            this._funnelSubMenu.label.text = funnels.length === 1
                ? _('Funnel (1)')
                : _fmt(_('Funnel (%d)'), funnels.length);
        ...
        _renderRoutes(snap) {
            ...
            this._routesSubMenu.label.text = n === 1
                ? _('Tailnet routes (1)')
                : _fmt(_('Tailnet routes (%d)'), n);
        ...
        _renderExitNodes(snap) {
            ...
            sub.removeAll();
            // Detach Allow LAN before clearing so reattaching works cleanly.
            if (this._allowLanRow.get_parent())
                this._allowLanRow.get_parent().remove_child(this._allowLanRow);
            ...
            if (snap.exitNodes.length === 0) {
                ...
                const hint = new InfoRow(_('Approve one in the admin console'));
                hint.reactive = false;
                hint._label.style_class = 'tailscale-peer-ip';
            ...
            // Allow LAN access only matters when an exit node is active. Park
            // it inside this submenu (under a separator) so it stops cluttering
            // the main toggle list.
            if (snap.exitNodeID) {
                sub.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
                sub.addMenuItem(this._allowLanRow);
            }
        }
