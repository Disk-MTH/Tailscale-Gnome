 ...
 */
async function _makeArchive(paths, password) {
    let dir;
    try {
    ...
    const first = GLib.path_get_basename(paths[0]).replace(/\.[^.]+$/, '');
    const archive = GLib.build_filenamev([dir, `${first || 'taildrop'}-${stamp}.zip`]);
...
// peer list so Nautilus and in-shell flows feel identical. Rows toggle a
// selection rather than sending on click: one file set often has to reach
// several devices, and a stray click on a list should not start a transfer.
const SendFileDialog = GObject.registerClass(
    class SendFileDialog extends ModalDialog.ModalDialog {
        _init({ files, peers, forceZip = false, onPick }) {
            super._init({ styleClass: 'tailscale-send-dialog' });
            this._onPick = onPick;
            this._resolved = false;
            this._asZip = forceZip;
            this._password = '';
            // Insertion-ordered, so recipients are sent to in the order they
            // were ticked and the toasts read predictably.
            ...
            this._buildZipControls(forceZip);
            ...
            // Kept so the send action can be greyed out while nothing is
            // ticked: Dialog checks `button.reactive` before firing, both on
            // click and on the Return binding that `default: true` installs.
        ...
        // is forced on and locked in that case rather than offering a
        // choice that would fail.
        _buildZipControls(forceZip) {
            const box = new St.BoxLayout({
                vertical: true,
                x_expand: true,
            ...
            const zipBtn = new St.Button({
                style_class: 'tailscale-send-row tailscale-zip-toggle',
                can_focus: !forceZip,
                x_expand: true,
                toggle_mode: true,
                checked: forceZip,
            ...
            const zipLine = new St.BoxLayout({ x_expand: true });
            const zipText = new St.BoxLayout({
                vertical: true,
                x_expand: true,
                y_align: Clutter.ActorAlign.CENTER,
            });
            zipText.add_child(new St.Label({ text: _('Send as zip') }));
            if (forceZip) {
                zipText.add_child(new St.Label({
                    text: _('Required: the selection contains a folder'),
                    style_class: 'tailscale-peer-ip',
                }));
            }
            zipLine.add_child(zipText);
            const zipMark = new St.Label({
                text: '\u2713',
                style_class: 'tailscale-zip-check',
                y_align: Clutter.ActorAlign.CENTER,
            });
            zipMark.visible = forceZip;
            zipLine.add_child(zipMark);
            zipBtn.set_child(zipLine);
            box.add_child(zipBtn);

            const pwBox = new St.BoxLayout({
                vertical: true,
                x_expand: true,
            ...
            pwBox.add_child(pwEntry);
            pwBox.add_child(pwHint);
            pwBox.visible = forceZip;
            box.add_child(pwBox);
            ...
            zipBtn.connect('notify::checked', () => {
                this._asZip = zipBtn.checked;
                zipMark.visible = zipBtn.checked;
                pwBox.visible = zipBtn.checked;
            });
        ...
        _makeRow(peer) {
            ...
            // now carries the same tick the zip toggle uses.
        ...
        _syncSendButton() {
            ...
            const any = this._selected.size > 0;
            this._sendButton.reactive = any;
            this._sendButton.can_focus = any;
        }
...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        ...
        _buildMenu() {
            ...
            // One entry, and the `shortcut-send-file` keybinding reaches the
            // very same call through Indicator.sendFiles(). The picker takes
            // files and folders together, so there is nothing left to choose
            // between beforehand — what the selection turns out to contain is
            // settled afterwards, in runSendFlow.
            this._sendFileRow = new ActionRow(_('Send files'),
        ...
        // menu open. Closing first matches what the menu row does, so the
        // dialog is never stacked under the Quick Settings popup.
        runAddFunnelFlow() {
            this._closeAllMenus();
            return this._runAddFunnelFlow();
        }
        ...
        async runSendFlow(preselectedFiles) {
            ...
            // Taildrop only moves plain files, so a directory anywhere in the
            // selection forces the archive path. A selection of plain files
            // leaves zipping optional, which is what the dialog then offers.
            const hasDir = files.some(
                (p) => GLib.file_test(p, GLib.FileTest.IS_DIR));
            ...
            this._showDialog(new SendFileDialog({
                ...
                onPick: (targets, opts) => {
                    ...
                    this._sendTo(targets, files, opts);
                },
            }));
        }
        ...
        async _sendTo(peers, files, { asZip, password }) {
            ...
            // nothing about which of the evening's transfers it was.
            const sentMsg = _fmt(_('Sent %s to %s'), label, where);
            ...
            await this._withFeedback(
                ...
                async () => {
                    ...
                    const archive = await _makeArchive(files, password);
                    if (!archive) {
                        return {
                            ok: false,
            ...
            return new Promise((resolve) => {
                ...
                bus.call(
                    ...
                    new GLib.Variant('(ssa{sv})', ['', title, {
                        ...
                        // `directory` does not restrict the picker to
                        // folders — it lets folders be picked *as well as*
                        // files, and several of each at once. One chooser
                        // therefore covers the whole send flow; runSendFlow
                        // works out what came back and forces the archive
                        // path only when a folder is actually in there.
        ...
        // the client so it can be translated, and the "click to open" half is
        // only promised when the live backend can actually honour a click —
        // toasts cannot (see Notifier.canActivate).
        _notifyFileReceived(path, size) {
            const name = path.split('/').pop();
            const human = GLib.format_size(size);
            const clickable = Notifier.canActivate;
            Notifier.notify({
                category: Category.TAILDROP,
                level: 'success',
                spontaneous: true,
                message: clickable
                    ? _fmt(_('Received %s (%s) - click to open'), name, human)
                    : _fmt(_('Received %s (%s)'), name, human),
                onActivate: clickable ? () => _showInFileManager(path) : null,
            });
        }
