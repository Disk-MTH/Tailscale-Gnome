...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        ...
        _buildMenu() {
            ...
            this._sendFolderRow = new ActionRow(_('Send folder'),
                () => {
                    this._closeAllMenus();
                    this.runSendFlow(null, { directory: true });
                });
            this.menu.addMenuItem(this._sendFolderRow);
        ...
        _applyFeatureGates() {
            ...
            this._sendFolderRow.visible   &&= taildrop;
        ...
        async runSendFlow(preselectedFiles, { directory = false } = {}) {
            let files = preselectedFiles;
            if (!files || files.length === 0) {
                files = await this._pickFiles(
                    directory
                        ? _('Send a folder via Taildrop')
                        : _('Send files via Taildrop'),
                    directory,
                );
                if (!files) return;
            }

            // Taildrop only moves plain files, so a directory in the
            // selection forces the archive path.
            ...
            return new Promise((resolve) => {
                ...
                bus.call(
                    ...
                    new GLib.Variant('(ssa{sv})', ['', title, {
                        ...
                        // The portal has no mixed mode: a picker selects
                        // files or folders, never both. Hence the separate
                        // menu entry rather than one combined chooser.
