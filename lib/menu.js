...
function _accountSubtitle(acc) {
    ...
    if (!account) return '';
    let kind;
    if (!tailnet || tailnet === account) kind = _('Personal');
    else if (tailnet.includes('@')) kind = _('Guest');
    else kind = _('Organisation');
...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        ...
        destroy() {
            ...
            // button we grafted onto it is not torn down with us. Without
            // this the next enable() would stack a second one beside it.
