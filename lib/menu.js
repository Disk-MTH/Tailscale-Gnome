...
export const TailscaleToggle = GObject.registerClass(
    class TailscaleToggle extends QuickSettings.QuickMenuToggle {
        ...
        // Cap the menu's vertical size and make its content scrollable.
        // QuickSettings adds `this.menu.actor` to its overlay; if the
        // actor is the raw `box` (an unbounded BoxLayout) the popup will
        // grow past the screen edge with no scrollbar. We redirect
        // `menu.actor` to a ScrollView that holds `box`, so the overlay
        // sees a bounded wrapper while item-management code can keep
        // using `menu.box` unchanged.
        _makeMenuScrollable() {
            const box = this.menu.box;
            if (!box || this._scrollWrap) return;

            const monitor = Main.layoutManager.primaryMonitor;
            const panelH = Main.panel?.height ?? 32;
            const maxH = monitor
                ? Math.floor((monitor.height - panelH * 2) * 0.80)
                : 600;
            ...
            // Reparent the items box under the ScrollView. If the system
            // already attached `box` somewhere (St.Bin uses set_child,
            // BoxLayout-like containers use add/remove_child), pull it
            // out cleanly first then add the ScrollView in its place.
            const oldParent = box.get_parent();
            if (oldParent) {
                if (typeof oldParent.set_child === "function" &&
                    oldParent.get_child?.() === box) {
                    oldParent.set_child(null);
                } else if (typeof oldParent.remove_child === "function") {
                    oldParent.remove_child(box);
                }
            }
            sv.set_child(box);
            if (oldParent) {
                if (typeof oldParent.set_child === "function" &&
                    oldParent.get_child?.() == null) {
                    oldParent.set_child(sv);
                } else if (typeof oldParent.add_child === "function") {
                    oldParent.add_child(sv);
                }
            }

            // Mirror the items box's visibility onto the wrapper. The
            // QuickSettings overlay shows/hides per-toggle pages by
            // toggling `menu.actor.visible`; some code paths capture the
            // original actor reference at setup time, so flipping `box`
            // must keep `sv` in sync (and vice-versa).
            box.bind_property("visible", sv, "visible",
                GObject.BindingFlags.SYNC_CREATE |
                GObject.BindingFlags.BIDIRECTIONAL);

            // Re-route `menu.actor` so anyone reading it sees the wrap.
            this.menu.actor = sv;
            this._scrollWrap = sv;
        }
